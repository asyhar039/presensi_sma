...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($username) || empty($password) || empty($nama)) {
            ...
            try {
                $hashed_pass = password_hash($password, PASSWORD_BCRYPT, ['cost' => 10]);
                
                // Insert user
                $stmt = $conn->prepare("INSERT INTO users (username, password, email, nama_lengkap, role) VALUES (?, ?, ?, ?, 'guru')");
                $stmt->bind_param("ssss", $username, $hashed_pass, $email, $nama);
                $stmt->execute();
                $user_id = $conn->insert_id;
                
                // Insert guru
                $stmt = $conn->prepare("INSERT INTO guru (user_id, nip, no_telp, alamat) VALUES (?, ?, ?, ?)");
                $stmt->bind_param("isss", $user_id, $nip, $telp, $alamat);
                $stmt->execute();
...
<head>
    ...
    <title>Tambah Guru - Sistem Absensi SMA</title>
